...
/// <summary>Полная хронология лечения/наблюдения ребёнка. Родитель видит своего ребёнка; специалист — только если есть/была бронь с этим ребёнком.</summary>
[ApiController]
[Route("api/parent/children/{childId:guid}/chronology")]
[Authorize(Roles = "Parent")]
public class ParentChildChronologyController : ControllerBase
{
    private readonly UserManager<ApplicationUser> _userManager;
    private readonly AppDbContext _db;
    private readonly IFamilyContextService _familyContext;

    public ParentChildChronologyController(UserManager<ApplicationUser> userManager, AppDbContext db, IFamilyContextService familyContext)
    {
    ...
    public async Task<ActionResult<IEnumerable<object>>> Get(Guid childId, [FromQuery] DateTime? fromUtc, [FromQuery] DateTime? toUtc)
    {
        ...
        var childOk = await _db.Children.AnyAsync(c => c.Id == childId && c.ParentProfileId == family.ParentProfileId);
        if (!childOk) return NotFound();

        return await BuildChronology(childId, fromUtc, toUtc);
    }

    protected async Task<ActionResult<IEnumerable<object>>> BuildChronology(Guid childId, DateTime? fromUtc, DateTime? toUtc)
    {
        ...
        foreach (var b in bookings)
        {
            items.Add(new
            {
                ...
                status = (int)b.Status,
                summary = $"Встреча {b.StartsAtUtc:yyyy-MM-dd HH:mm}"
            });
            if (b.Outcome != null)
                items.Add(new
                {
                    ...
                    id = b.Outcome.Id,
                    bookingId = b.Id,
                    atUtc = b.Outcome.CreatedAtUtc,
                    summary = b.Outcome.Summary != null && b.Outcome.Summary.Length > 300 ? b.Outcome.Summary[..300] + "…" : (b.Outcome.Summary ?? "")
                });
        }

        // Daily entries: дата дня
        ...
        foreach (var e in entries)
            items.Add(new
            {
                ...
                summary = e.ParentNote != null && e.ParentNote.Length > 200 ? e.ParentNote[..200] + "…" : (e.ParentNote ?? "Запись за день")
            });

        // Медикаменты (по времени приёма)
        var entryIds = entries.Select(e => e.Id).ToList();
        if (entryIds.Count > 0)
        {
            var meds = await _db.DailyMedIntakes.AsNoTracking()
                .Where(m => entryIds.Contains(m.DailyEntryId))
                .Select(m => new { m.Id, m.Drug, m.Dose, m.TimeUtc, m.Taken })
                .ToListAsync();
            foreach (var m in meds)
            {
                ...
                items.Add(new
                {
                    ...
                    id = m.Id,
                    atUtc = m.TimeUtc,
                    summary = $"{m.Drug}" + (m.Dose != null ? $" {m.Dose}" : "") + (m.Taken ? " (принято)" : " (не принято)")
                });
            }

            var sessions = await _db.DailySessions.AsNoTracking()
                .Where(s => entryIds.Contains(s.DailyEntryId))
                .Select(s => new { s.Id, s.Type, s.DurationMin, s.CreatedAtUtc })
                .ToListAsync();
            ...
            var incidents = await _db.DailyIncidents.AsNoTracking()
                .Where(i => entryIds.Contains(i.DailyEntryId))
                .Select(i => new { i.Id, i.TimeUtc, i.Intensity, i.Notes })
                .ToListAsync();
            foreach (var i in incidents)
            {
                ...
                items.Add(new
                {
                    ...
                    summary = i.Notes != null && i.Notes.Length > 150 ? i.Notes[..150] + "…" : (i.Notes ?? $"Инцидент (интенсивность {i.Intensity})")
                });
            }
        }
